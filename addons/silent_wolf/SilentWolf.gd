...
func sw_save_high_scores(player_name: String, ldboard_name: String='main', plus: int = 0):
	...
	if sw_score.is_empty():
		SilentWolf.Scores.save_score(player_name, plus , ldboard_name)
	else:
		SilentWolf.Scores.save_score(player_name, sw_score[0]["score"] + plus , ldboard_name)
