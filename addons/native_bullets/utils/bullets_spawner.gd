...
func shoot(recover_seconds = 0.0):
	...
	for spawner in children:
		...
		Bullets.set_bullet_property(bullet_id, "transform", Transform2D(bullet_rotation, spawner.global_position \
				+ bullet_velocity * recover_seconds \
				+ bullet_velocity.normalized() * bullets_spawn_distance))
		Bullets.set_bullet_property(bullet_id, "velocity", Vector2(0, 1) * bullets_speed)
		Bullets.set_bullet_property(bullet_id, "rotate", false)
		Bullets.
