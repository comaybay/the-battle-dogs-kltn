...
func enter(data: Dictionary) -> void:
	super.enter(data)
	var danamku_space := InBattle.get_danmaku_space() as DanmakuSpace
	var flower_kit := Danmaku.get_bullet_kit(BulletKits.FLOWER)
	...
	pattern.start(1, func(_pos: Vector2, angle: float, _rdelta: float, index: int):
		...
		bullet.start(func():
			...
			tween.tween_callback(func():
				if bullet.is_destroyed(): tween.kill()	
			)
		)
	)
